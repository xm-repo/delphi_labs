...
begin
  ...
  if hDll = 0 then
  begin
    MessageBox(Self.Handle, PChar('Не удалось загрузить ' + DllFile), PChar('Ошибка'), MB_OK + MB_ICONERROR);
    Exit;
  end;
  ...
  begin
    MessageBox(Self.Handle, PChar('FloatFunc не найдена в ' + DllFile), PChar('Ошибка'), MB_OK + MB_ICONERROR);
    Exit;
  end;
...
begin
  ...
  begin
    MessageBox(Self.Handle, PChar('StrFunc не найдена в ' + DllFile), PChar('Ошибка'), MB_OK + MB_ICONERROR);
    Exit;
  end;
